...
export async function serviceRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get('/v1/services', async (request, reply) => {
    ...
    const filters = {
      ...
      minPrice: query.minPrice && Number.isFinite(parseFloat(query.minPrice)) ? parseFloat(query.minPrice) : undefined,
      maxPrice: query.maxPrice && Number.isFinite(parseFloat(query.maxPrice)) ? parseFloat(query.maxPrice) : undefined,
      q: query.q || undefined,
      limit: Math.min(Math.max(1, parseInt(query.limit || '20', 10) || 20), 100),
      offset: Math.max(0, parseInt(query.offset || '0', 10) || 0),
